...
use crate::theme::{animation::AnimationState, colors};
...
impl<'a> Launcher<'a> {
    fn render_input_bar(&self, area: Rect, buf: &mut Buffer, state: &LauncherState) {
        ...
        let cursor_char = if self.animation.cursor_visible() {
            "▎"
        } else {
            " "
        };
        ...
        let count_text = format!(" {} results", state.result_count());
        ...
        if inner.width > count_width + 10 {
            ...
            Paragraph::new(Span::styled(count_text, colors::text_dim())).render(count_area, buf);
        }
    }
    ...
    fn render_results_list(&self, area: Rect, buf: &mut Buffer, state: &mut LauncherState) {
        ...
        let fuzzy_list = FuzzyList::new(&state.results).block(block);
    ...
    fn render_preview(&self, area: Rect, buf: &mut Buffer, state: &LauncherState) {
        if let Some(action) = state.selected_action() {
            ActionPreview::new(action).render(area, buf);
        } else {
            let block = Block::default()
                .title(Span::styled(
                    " Preview ",
